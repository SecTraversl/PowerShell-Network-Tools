...
.EXAMPLE
  ...
  Here we first run the function using the built-in alias of "ipaddress" and then we run the function using its full name. In return, we get fundamental information about the two active interfaces.  There are additional properties besides "IPAddress", "InterfaceAlias", "DefaultIPGateway", and "InterfaceIndex", but, by default, these are the only 4 properties that are retrieved (to get all of the properties use the '-Full' switch parameter).

.EXAMPLE
  PS C:\> Get-IPConfig -Full
  ...
  Here we run the function and display all of the returned properties by using the "-Full" switch parameter.  This presents the true value of this function in that information about DHCP, DNS, the IP Address, network/subnetmask, default gateway, MAC Address, and interface link speed are all gathered as properties of a single object.
...
.NOTES
  ...
  Author:  Travis Logue
  Version History:  2.3 | 2021-11-24 | Added a "-Full" switch to retrieve all pertinent information and added alias 'IPConfigGet'
  Dependencies: 
  Notes:
  ...
  - THING TO CONSIDER ADDING LATER:
     -CimSession
     -ComputerName
     -PSSession
  .
